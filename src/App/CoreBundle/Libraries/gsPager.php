...
class gsPager extends sfPager
{
    ...
    public function setQuery($query)
    {
        if (!$query instanceof $this->query_class) {
            throw new \InvalidArgumentException(
                'The given query is not ad instance of '.$this->query_class.', but '.\get_class($query)
            );
        }
        $this->query = $query;
    }
    ...
    public function getResults()
    {
        ...
        $q = $this->cloneQuery();
        $this->results = $q->setMaxResults($this->getMaxPerPage())->setFirstResult(
